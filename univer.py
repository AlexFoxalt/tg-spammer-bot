...
9. Используй список последних сообщений чтобы меньше повторяться.
10. Если невозможно сгенерировать сообщение в рамках правил, верни короткое корректное альтернативное напоминание, тоже как обычный текст.

Примеры подходящих выходов (вводишь их в промт как ориентир, но они не обязательны):
...
def get_last_messages(k: int = 5):
    with open("last_messages.txt", "r") as f:
        return f.readlines()[-k:]


def gen_message():
    upd_prompt = prompt.replace(
        "<last_messages>", "".join(get_last_messages(k=5))
    ).replace("<k>", str(5))
